    ...
    <div class="content-wrapper">
      <div class="content">
        ...
        <p class="confirm-delete-description">
          {{ description || $t('if_you_continue_with_this_action_the_resource_will_be_permanently_deleted') }}
          <br>
          <br>
          {{ $t('type_the_word') }}
        ...
        <InputText v-model="verifyWord" class="uppercase text-center" @keyup.enter="handlerConfirmDelete" />

        <p v-if="isIncorrect" class="incorrect">
          {{ $t('incorrect_value_to_confirm') }}
        </p>
        <div class="tools">
          <Button class="btn btn-block btn-danger" @click="handlerConfirmDelete">
            {{ $t('confirm_delete') }}
          </Button>
