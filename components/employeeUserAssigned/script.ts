  ...
  methods: {
    ...
    addNew() {
      if (this.employee.employeeId) {
        const newUserAssignedEmployee: UserResponsibleEmployeeInterface = {
        ...
        this.userAssignedEmployee = newUserAssignedEmployee
    ...
    async confirmDelete() {
      ...
      if (this.userAssignedEmployee) {
        ...
        if (userAssignedEmployeeResponse.status === 200) {
          ...
          this.$toast.add({
            severity: 'success',
            summary: 'Delete user assigned employee',
