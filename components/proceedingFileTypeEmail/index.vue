<template>
  <div v-if="isReady" class="proceeding-file-type-emails">
    ...
    <h1>
      {{
      `${capitalizeFirstLetter($t(proceedingFileType.proceedingFileTypeName.toLocaleLowerCase()))}`
      }}
    </h1>
    ...
    <div v-if="isReady">
      <div class="proceeding-file-type-emails-wrapper">
        <div v-for="(proceedingFileTypeEmail, index) in proceedingFileTypeEmails"
          :key="`proceeding-file-type-email-${index}`">
        ...
        <h5 v-if="legacyProceedingFileTypeEmails.length > 0">{{ $t('legacy_emails') }}</h5>
        <div v-for="(proceedingFileTypeEmail, index) in legacyProceedingFileTypeEmails"
          :key="`legacy-proceeding-file-type-emaiL-${index}`">
          <span>{{ proceedingFileTypeEmail.proceedingFileTypeEmailEmail }}</span>
        </div>
      </div>
      <transition name="page">
        <confirmDelete v-if="drawerProceedingFileTypeEmailDelete" @confirmDelete="confirmDelete"
