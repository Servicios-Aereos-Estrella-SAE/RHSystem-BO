...
import type { MenuItemInterface } from '~/resources/scripts/interfaces/MenuItemInterface'
...
export default defineComponent({
  ...
  async created() {
    ...
    if (systemModuleResponse.status === 200) {
      ...
      await this.assignItemsMenu(systemModules, 'Attendance Monitor Reporter')
      await this.assignItemsMenu(systemModules, 'Organization')
      await this.assignItemsMenu(systemModules, 'Shifts Control')
      await this.assignItemsMenu(systemModules, 'System Control')
      await this.assignItemsMenu(systemModules, 'Managed Aircraft')
    }
  },
  methods: {
