<template>
  <div class="box flight-attendant-info-card">
    <div class="avatar" @click="onClickPhoto">
      <img v-if="flightAttendant.flightAttendantPhoto" :src="flightAttendant.flightAttendantPhoto"
        alt="Flight Attendant Photo" class="flight-attendant-photo" />
    ...
    <div class="name">
      {{ formatName(flightAttendant.employee.person?.personFirstname) }}
      {{ formatName(flightAttendant.employee.person?.personLastname) }}
      {{ formatName(flightAttendant.employee.person?.personSecondLastname) }}
    </div>
    <div class="employee-number">
      {{ `Emp. ID: ${flightAttendant.employee.employeeCode}` }}
    </div>
    <div class="line">
    </div>
    <div class="info business-unit">
      {{ `${ $t('business_unit') }: ${flightAttendant.employee.businessUnit?.businessUnitName || 'Not assigned'}` }}
    </div>
    <div class="info capitalize">
      {{ `${flightAttendant.employee?.department?.departmentName || ''}`}}
    </div>
    <div class="info">
      {{ `${flightAttendant.employee?.position?.positionName || ''}` }}
    </div>
    <div class="box-tools-footer">
      ...
      <Button v-if="canDelete" class="btn btn-red" @click="handlerClickOnDelete()">
        <svg fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path
