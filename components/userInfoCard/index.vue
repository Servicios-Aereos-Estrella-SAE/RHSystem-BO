<template>
  <div class="box user-info-card">
    ...
    <div v-if="user.person" class="name capitalize">
      {{ `${user.person.personFirstname || ''}`.toLocaleLowerCase() }} {{ `${user.person.personLastname ||
        ''}`.toLocaleLowerCase() }} {{ `${user.person.personSecondLastname || ''}`.toLocaleLowerCase() }}
    </div>
    ...
    <div v-if="getDepartment" class="info capitalize">
      {{ getDepartment }}
    </div>
    <div v-else class="info">
      ---
    </div>
    <div v-if="getPosition" class="position capitalize">
      {{ getPosition }}
    </div>
    <div v-else class="info">
      ---
    </div>
    <div v-if="getEmployeeNumber" class="number">
      {{ `No. Emp: ${getEmployeeNumber}` }}
    </div>
    <div v-else class="info">
      ---
    </div>
