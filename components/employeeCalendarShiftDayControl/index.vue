<template>
  <div class="caledar-day">
    <div v-if="isReady">
      ...
      <div class="shift">
        <Dropdown
          v-if="drawerEmployeeShiftForm && employeeShift && shiftEditSelected && (shiftEditSelected.day === employeeCalendar.day)"
          v-model="employeeShift.shiftId" :options="shiftsList" optionLabel="shiftName" optionValue="shiftId" filter />
        <div v-else>
          <span
            v-if="!employeeCalendar.assist.dateShift || employeeCalendar.assist.isRestDay || employeeCalendar.assist.isVacationDate || employeeCalendar.assist.isWorkDisabilityDate"
            class="off">
            ---- -- ----
          </span>
          ...
          <div class="icon">
            <svg viewBox="0 0 64 64" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
              <path
                ...
                fill="#88a4bf" class="fill-241f20"></path>
              <circle cx="24.547" cy="5.627" fill="#88a4bf" r="5.594" class="fill-241f20"></circle>
              <g fill="#88a4bf" class="fill-241f20">
                <path
                  d="m28.006 30.869.009.134M40.076 43.046H23.262c-.095 0-.188 0-.281-.015h-.14a4.824 4.824 0 0 1-4.982-4.498l-.719-10.975c-5.425 3.534-9.018 9.643-9.018 16.601 0 10.94 8.869 19.808 19.809 19.808 7.716 0 14.383-4.419 17.655-10.855l-5.51-10.066z">
                </path>
              </g>
            </svg>
          </div>
      ...
      <div class="tools">
        <Button
          v-if="displayButtonManageShift"
          id="btn-open-shift-form"
          class="btn btn-block"
          :disabled="isDeleted || !canManagementShift"
          @click="handlerShiftForm(employeeCalendar)" >
        ...
        <Button
          v-if="displayButtonManageExceptions"
          id="btn-open-exceptions"
          class="btn btn-block"
          :class="{ active: employeeCalendar.assist.hasExceptions }"
          :disabled="!employeeCalendarAssist.assist.dateShift"
          @click="handlerClickExceptions" >
        ...
        <Button
          ...
          class="btn btn-block"
          :class="{ active: employeeCalendar?.assist?.dateShift?.shiftIsChange }"
          :disabled="!employeeCalendarAssist.assist.dateShift"
          @click="handlerClickShiftChanges" >
          <svg fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m15.207 2.292 4 3.995a1 1 0 0 1 .084 1.32l-.083.094-4 4.006a1 1 0 0 1-1.498-1.32l.083-.094L16.083 8H5.5a1 1 0 0 1-.994-.883L4.5 7a1 1 0 0 1 .883-.993L5.5 6h10.59l-2.296-2.293a1 1 0 0 1-.084-1.32l.083-.095a1 1 0 0 1 1.32-.084l.094.084 4 3.995-4-3.995Zm4.283 14.591.007.117a1 1 0 0 1-.883.993l-.117.007H7.913l2.294 2.293a1 1 0 0 1 .084 1.32l-.083.094a1 1 0 0 1-1.32.084l-.095-.084-4-3.996a1 1 0 0 1-.083-1.32l.083-.094 4-4.004a1 1 0 0 1 1.498 1.32l-.083.094L7.918 16h10.579a1 1 0 0 1 .993.883l.007.117-.007-.117Z" fill="#88a4bf" class="fill-212121"></path></svg>
        </Button>

        <Button
          v-if="displayAcceptEditShiftButton"
          id="btn-store-shift"
          class="btn btn-block"
          @click="onSave">
        ...
        <Button
          v-if="displayCancelEditShiftButton"
          id="btn-cancel-shift-form"
          class="btn btn-block"
          @click="handlerCancelEditShift">
