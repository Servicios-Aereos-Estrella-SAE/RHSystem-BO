<template>
  <div class="box pilot-info-card">
    <div class="avatar" @click="onClickPhoto">
      <img v-if="pilot.pilotPhoto" :src="pilot.pilotPhoto" alt="Pilot Photo" class="pilot-photo" />
    ...
    <div class="name">
      {{ formatName(pilot.employee.person?.personFirstname) }}
      {{ formatName(pilot.employee.person?.personLastname) }}
      {{ formatName(pilot.employee.person?.personSecondLastname) }}
    </div>
    <div class="employee-number">
      {{ `Emp. ID: ${pilot.employee.employeeCode}` }}
    </div>
    <div class="line">
    </div>
    <div class="info business-unit">
      {{ `${ $t('business_unit') }: ${pilot.employee.businessUnit?.businessUnitName || 'Not assigned'}` }}
    </div>
    <div class="info capitalize">
      {{ `${pilot.employee?.department?.departmentName || ''}`}}
    </div>
    <div class="info">
      {{ `${pilot.employee?.position?.positionName || ''}` }}
    </div>
