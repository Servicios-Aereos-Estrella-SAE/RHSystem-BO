<template>
  <div class="work-disability-info-card">
    ...
    <div v-if="workDisability.workDisabilityPeriods.length > 0" class="insurance-coverage-period">
      <div v-for="(workDisabilityPeriod, index) in workDisability.workDisabilityPeriods"
        :key="`workDisabilityPeriods-${workDisabilityPeriod.workDisabilityPeriodId}-${index}`" class="period-applied">
    ...
    <div v-if="!isDeleted" class="box-tools-footer">
      ...
      <Button v-if="displayDestroyButton && canManageUserResponsible" class="btn btn-block"
