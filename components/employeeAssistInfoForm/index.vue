<template>
  <div class="employee-assist-info-form">
    <employeeModalInfoCard v-if="employee" :employee="employee" />

    <h2>
      Add employee manual assist
    </h2>

    <Message :closable="false">
      Adding an attendance record can be a delicate action.
      <br>
      You should be very sure before doing so.
    </Message>
      ...
      <div class="form-container">
        <div class="input-box">
          <div class="date-box-container">
            <label>Select date time to assign assist</label>
            <div v-if="!displayDateCalendar" class="date-box">
              <InputText v-model="assistPunchTime" readonly class="capitalize" disabled />
            ...
            <div v-if="displayDateCalendar" class="date-box-controller">
              <Calendar
                v-if="displayDateCalendar"
                dateFormat="yy-mm-dd"
                showTime
                hourFormat="24"
                v-model.lazy="assist.assistPunchTime"
                placeholder="Select date time to assist"
                :invalid="dateInvalid"
        ...
        <div class="box-tools-footer">
          <Button label="Save" class="btn btn-block btn-primary" @click="onSave" />
        </div>
      </div>
    </div>
  </div>
