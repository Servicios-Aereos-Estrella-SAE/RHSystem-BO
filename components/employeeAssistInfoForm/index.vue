<template>
  <div class="employee-assist-info-form">
    ...
    <h2>
      {{ $t('add_employee_manual_assist') }}
    </h2>

    <Message :closable="false">
      {{ $t('adding_an_attendance_record_can_be_a_delicate_action') }}
      <br>
      {{ $t('you_should_be_very_sure_before_doing_so') }}
    </Message>
      ...
      <div class="form-container">
        <div class="input-box">
          <div class="date-box-container">
            <label>{{ $t('select_date_time_to_assign_assist') }}</label>
            ...
            <small class="p-error" v-if="dateInvalid">{{ $t('date_is_invalid') }}</small>
          </div>
        </div>
        <div class="box-tools-footer">
          <Button :label="$t('save')" class="btn btn-block btn-primary" @click="onSave" />
        </div>
      </div>
    </div>
  </div>
