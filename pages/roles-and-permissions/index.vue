      ...
      <div class="roles-and-permissions-page">
        <div class="module-permissions-wrapper">
          <div class="box roles-wrapper">
            ...
            <div v-for="(rol, rIndex) in roleList" :key="`role-${rIndex}-${rol.roleId}`" class="role" :class="{ active: roleSelected === rIndex }" @click="handlerSelectRole(rIndex)">
              <div class="role-name">
                {{ rol.roleName }}
              </div>
              <div class="buttons">
                ...
                <button v-if="activeEdit" class="btn btn-to-edit" @click="onSave">
                  <svg fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m8.5 16.586-3.793-3.793a1 1 0 0 0-1.414 1.414l4.5 4.5a1 1 0 0 0 1.414 0l11-11a1 1 0 0 0-1.414-1.414L8.5 16.586Z" fill="#88a4bf" class="fill-212121 fill-ffffff"></path></svg>
                </button>
              </div>
            </div>
          </div>
          <div class="box">
            ...
            <div class="modules-wrapper">
              <div v-for="(sModule,  smIndex) in systemModulesList" :key="`system-module-${smIndex}-${sModule.systemModuleId}`" class="module">
                <h3>
                  {{ sModule.systemModuleName }}
                </h3>
                <div class="permissions-wrapper">
                  <div v-for="(permission, indexP) in sModule.systemPermissions" :key="`permission-${indexP}`" class="permission">
                    <Checkbox v-model="permissions[roleSelected]" name="permission" :disabled="!canUpdate || !activeEdit" :value="permission.systemPermissionId" />
                    <label>
          ...
          <div class="box">
            ...
            <div class="departments-wrapper">
              <div v-for="(depto, gdIndex) in departmentList" :key="`group-department-${gdIndex}`" class="depto">
                <Checkbox v-model="departmentPermissions[roleSelected]" name="permission" :disabled="!canUpdate || !activeEdit" :value="depto.departmentId" />
                <label>
                    ...
                    <Column header="Permissions">
                      <template #body="slotProps">
                        <div class="permissions">
                          <div v-for="(permission, indexP) in slotProps.data.systemPermissions"
                            :key="`permission-${indexRole}-${indexP}`" class="permission">
                            <Checkbox v-model="permissions[indexRole]" name="permission" :disabled="!canUpdate"
                              :value="permission.systemPermissionId" />
                    ...
                    <Column v-for="(dept, index) in groupedDepartments[0]" :key="index" header="Department">
                      <template #body="slotProps">
                        <div v-if="slotProps.data[index]">
                          <Checkbox v-model="departmentPermissions[indexRole]" name="permission" :disabled="!canUpdate"
